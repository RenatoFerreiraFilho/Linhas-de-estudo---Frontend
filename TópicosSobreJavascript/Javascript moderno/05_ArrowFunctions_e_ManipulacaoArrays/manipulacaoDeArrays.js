...
// 3 - map => retorna array
let names = usPresidents.map((item, index) => `${index} - ${item.president}`)
console.log(names)

//OBS: map fazendo trabalho de filter:
let mapFazendoFilter = []
usPresidents.map((pres,ind) => {
    if (pres.party == "Republican"){
        mapFazendoFilter = [...mapFazendoFilter,pres]
    }
})

// 4 - filter => retorna array
let presidentesPartidoRepublicano = usPresidents.filter((item) => item.party == "Republican")//muito mais simples
console.log(presidentesPartidoRepublicano)
// usando filter e map simultaneamente:
let nomesPresidentesPartidoRepublicano = usPresidents.filter((item) => item.party == "Republican").map((president) => president.president)
console.log(nomesPresidentesPartidoRepublicano)


// 5 - find
let p1 = usPresidents.find((item) => item.party == "Democratic")//vai encontrar sempre o primeiro elemento do array ou nenhum (retorna undefined)
console.log(p1)

// 6 - sort
usPresidents.sort((i1, i2) => {
    if (i1.president < i2.president){//vou ordenar por ordem alfabetica. Se eu usar um número/data, vai entender também
        return -1//se for menor, suba
    }else if (i1.president > i2.president){
        return 1//se for maior, desça
    }else{
        return 0//se for igual, fique onde está
    }
})
console.log(usPresidents)
...
usPresidents.sort((i1, i2) => {
    if (i1.president > i2.president){//vou ordenar por ordem alfabetica. Se eu usar um número/data, vai entender também
        return -1//se for maior, suba
    }else if (i1.president < i2.president){
        return 1//se for menor, desça
    }else{
        return 0//se for igual, fique onde está
    }
})
